...
if __name__ == "__main__":
    ...
    #
    # 5,10
    shape = Rectangle(d=10*cm, b=3*cm, mesh_scale=1/400)
    I = shape.cmm()[1,1]
    A = shape.cnn()[0,0]
            ...
            for shear in [0,1]:
                for warp in True, False:
                    ...
                    model = prism.create_model(
                        echo_file=open(f"echo_{element}_{section}_{shear}_{warp}.tcl", "w")
                    )
                    u, P = analyze(model, prism, w)
                    ax.plot(u, P, next(markers), label=f"{element}, {section}, shear: {bool(shear)}, warp: {warp}")
