...
proc printEigenvalues {E A I L tolerance} {

    # Compute element eigenvalues
    set eigenValues [eigen standard -symmBandLapack 6]
    ...
    for {set i 0} {$i < 3} {incr i} {
        verify value  [lindex $eigenValues [expr $i+3]]  [lindex $exact $i] $tolerance "Eigenvalue [expr $i+1]"
    }
}

proc printDisplacements {E A I L P H M tolerance} {
    verify error  [nodeDisp 2 1]  [expr $P*$L/($E*$A)] $tolerance
    verify error  [nodeDisp 2 2]  [expr $H*pow($L,3)/(3*$E*$I) + $M*pow($L,2)/(2*$E*$I)] $tolerance
    verify error  [nodeDisp 2 3]  [expr $H*pow($L,2)/(2*$E*$I) + $M*$L/($E*$I)] $tolerance
}
set E 1.0
set b 2.0
set d 12.0
  ...
  foreach section $sections {
    ...
    switch $section {
        ...
        Fiber {
            ...
            # This section is less accurate, so we relax the tolerance
            set tolerance 0.1
        }
        AxialFiber {
            puts "  Section: $section"
            uniaxialMaterial Elastic 1 $E
            section AxialFiber 1 {
                set b2 [expr $b/2]
                set d2 [expr $d/2]
                patch rect 1 20 1 $d2 $b2 -$d2 -$b2
            }
    ...
    printEigenvalues $E $A $I $L $tolerance
    ...
    printDisplacements $E $A $I $L $P $H $M $tolerance
