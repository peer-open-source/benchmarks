...
def test_gradient():
    ...
    if True:
        ...
        assert _is_close(model.nodeDisp(2, 2), H*L**3/(3*E*I))
        assert _is_close(model.sensNodeDisp(2, 2, 1), -H*L**3/(3*E**2*I))
        assert _is_close(model.sensNodeDisp(2, 2, 2), 0)
        assert _is_close(model.sensNodeDisp(2, 2, 3), -H*L**3/(3*E*I**2))
        assert _is_close(model.sensNodeDisp(2, 2, 4), H*L**2/(E*I))
    ...
    print("u",      H*L**3/(3*E*I))
    print("du/dE", -H*L**3/(3*E**2*I))
    print("du/dA",  0)
    print("du/dI", -H*L**3/(3*E*I**2))
    print("du/dL",  H*L**2/(E*I))
    print("du/dH",    L**3/(3*E*I))
