...
class ColumnAnalysis:
    ...
    def create_model(self):
        ...
        with open(Path(__file__).parents[0]/"mesh1.json", "r") as f:
            fibers = json.load(f)
    ...
    def analyze(self):
        ...
        model.test("Energy", 1.0e-18, 20, 2)
        ...
        for sgn in 1, -1, 1:
            ...
            print(dU, maxU)
            dU1, dU2 = dU/20, dU*2.
            model.integrator("DisplacementControl", (nn,1), dU, 4, dU1, dU2)
            ...
            while status == ops.successful and sgn*u[-1]*L < sgn*maxU:
                ...
                if status != ops.successful:
                    ...
                    model.test("Energy", 1.0e-14, 50, 1)
                    # model.algorithm("NewtonLineSearch", 0.8)
                    model.algorithm("ModifiedNewton", initial=True)
                    status = model.analyze(1)
                    if status == ops.successful:
    ...
    #               model.test("NormUnbalance", 1.0e-8, 100)
                    model.algorithm("NewtonLineSearch", 0.8)
                    model.test("Energy", 1.0e-16, 20, 2)
...
#   veux.serve(artist)
    ...
    c = ColumnAnalysis(2, shear=1, warping=True, concrete="Faria")
    ax.plot( *c.analyze() , '.-', label="Shear")
    # c = ColumnAnalysis(2, shear=0, warping=True, concrete="Faria")
