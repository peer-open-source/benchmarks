...
def buildAnalysis(model, integratorCommand, algoCmd, linear=False):

    model.constraints('Plain')
    model.numberer('Plain')
    model.eval(integratorCommand)
    if linear:
        model.test('NormDispIncr', 1.0e-4, 2, 0)
    else:
...
for integratorName, integratorCmd in integratorCmds:
    ...
    buildAnalysis(model, integratorCmd, "algorithm Newton", linear=True)
...
for algoName, algoCmd in algorithmCmds:
    ...
    buildAnalysis(model,integratorCmd, algoCmd, linear=False)
