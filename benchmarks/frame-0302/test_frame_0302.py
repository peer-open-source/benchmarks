import xara
import jax
from xara.para import ParaModel, ChainModel
...
def test_para():
    ...
    _print(("u"), uy,  H*L**3/(3*E*I))
    _print(("u", "dE"), dE, -H*L**3/(3*E**2*I))
    _print(("u", "dA"), dA, 0)
    _print(("u", "dI"), dI, -H*L**3/(3*E*I**2))

    dudI = jax.grad(lambda I: H*L**3/(3*E*I))(I)
    dAdp, dIdp = jax.jacfwd(_section_properties, (0,1))(tw, hw, bf, tf)
    _print(("u", "dL"), dL, H*L**2/(E*I))
    _print(("u", "tw"), dtw, dudI*dIdp[0])
    _print(("u", "hw"), dhw, dudI*dIdp[1])



def _run_chain(element="PrismFrame"):
    L = 5.0
    H = 300e3  # Load applied at the top node
    ...
    def create_model(L, E, A, Iz):
        ...
        m = ChainModel(ndm=3, ndf=6, parameters=4)
...
def test_chain():
    for element in ["PrismFrame", "ForceFrame"]:
        _run_chain(element)
