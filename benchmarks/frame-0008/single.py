    ...
    for element in ["ExactFrame", "ForceFrame"]:
        for a in [0.5]: #, 0.5, 0.75]:
            ...
            for node in model.getNodeTags():
                print(model.nodeDisp(node))
            artist = veux.create_artist(model, vertical=3, model_config={
                "extrude_outline": shape
            })
            artist.draw_outlines()
            # artist.draw_nodes()
            artist.draw_surfaces(position=model.nodeDisp,
                                 rotation=model.nodeRotation,scale=5e5)
            veux.serve(artist)
